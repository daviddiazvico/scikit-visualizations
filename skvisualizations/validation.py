...
def classifier_scatter(X, y, fname, pca_n_components=2, **kwargs):
    ...
    **kwargs: optional savefig named args
    ...
    names.append("class")
    data = pd.DataFrame(data=np.append(X, np.reshape(y, (len(y), 1)), axis=1), columns=names)
    sns.set()
...
def regressor_scatter(X, y, preds, fname, **kwargs):
    ...
    **kwargs: optional savefig named args
    ...
    data_y = np.append(data_y, np.reshape([0] * len(y), (len(y), 1)), axis=1)
    data_preds = np.append(X, np.reshape(preds, (len(preds), 1)), axis=1)
    data_preds = np.append(data_preds, np.reshape([1] * len(preds), (len(preds), 1)), axis=1)
    data = pd.DataFrame(data=np.append(data_y, data_preds, axis=0), columns=names)
    sns.set()
    sns.lmplot(x="X", y="y", hue="source", data=data)
    plt.savefig(fname, **kwargs)


def metaparameter_plot(search, param, fname, score="score", log_scale=True, **kwargs):
    """Metaparameter plot.
    ...
    **kwargs: optional savefig named args
    ...
    plt.plot(param_range[search.best_index_], test_mean[search.best_index_], "o", label="Best", color="r")
    plt.plot(param_range[search.best_index_], train_mean[search.best_index_], "o", color="r")
