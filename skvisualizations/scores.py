...
def scores_table(datasets, estimators, scores, stds=None, greater_is_better=True, method="average"):
    """Scores table.
    ...
    """
    ranks = np.asarray(
        [rankdata(-m, method=method) if greater_is_better else rankdata(m, method=method) for m in scores]
    )
    table = pd.DataFrame(data=scores, index=datasets, columns=estimators)
    for i, d in enumerate(datasets):
        for j, e in enumerate(estimators):
            table.loc[d, e] = scores[i, j]
            if stds is not None:
                table.loc[d, e] += stds[i, j]
            table.loc[d, e] += ranks[i, j]
...
def hypotheses_table(
    ...
    if multitest is not None:
        multitest_table = pd.DataFrame(index=[multitest], columns=["p-value", "Hypothesis"])
    ...
    if correction is not None:
        reject, pvalues, alphac_sidak, alphac_bonf = multipletests(pvalues, alpha, method=correction)
    else:
        reject = ["Rejected" if pvalue <= alpha else "Not rejected" for pvalue in pvalues]
    test_table = pd.DataFrame(index=comparisons, columns=["p-value", "Hypothesis"])
